...
export default class Sysinfo extends Command {
	...
	async onCall(thread, message, reply, api) {
		if (message.args.length == 0) message.args.push("server")
		...
		const {system, os, cpu, graphics} = isServer
			? this.data
			: await api.socket.emitPromise("sysinfo.static")
		const {uptime} = isServer
			? await si.time()
			: await api.socket.emitPromise("sysinfo.time")
		const memory = isServer
			? await si.mem()
			: await api.socket.emitPromise("sysinfo.mem")
		const botMemoryUsage = isServer
			? process.memoryUsage()
			: await api.socket.emitPromise("memoryUsage")
		...
		return [
			...
			`${graphics.controllers
				.map((g, index) => `GPU ${index + 1}: ${g.vendor} ${g.model}`)
				.join("\n")}`,
			`Usage:  ${Math.floor(
				botMemoryUsage.heapTotal / 1024 / 1024
			)}MB / ${Math.ceil(memory.total / 1024 / 1024)}MB (${Math.floor(
				percent
			)}%)`
		].join("\n")
	}
